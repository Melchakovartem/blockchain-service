...
RSpec.describe "Create campaign" do
  ...
  context "prfile exst" do
    ...
    before do
      ...
      post create_campaign_v1_advertisers_path, params: { profile_id: profile_id, campaign_id: campaign_id, 
                                                         token_amount: amount}
    end

    it "checks token amount of campaign" do
      expect(deal_service.get_token_amount(campaign_id)).to eq(amount)
    end
